extends BossState
class_name CharlotteState_Phase1
...
func _process(delta):
	...
	if(boss.HP < boss.phaseSections[boss.sectionIndex]):
		boss.sectionIndex -= 1;
		boss.changeState(CharlotteState_Phase2.new());
		pass
...
func moveTowardsKeyPoint(pointIndex):
	...
	if abs(TARGET - boss.position).length() > 3.0:
		...
		var pattern = Utils.createObject("res://objects/bullet-patterns/burstPatern.tscn",boss.position + Vector2(-95.0,-40.0));
		pattern.SPEED = 12.0;
		stopped = true;
